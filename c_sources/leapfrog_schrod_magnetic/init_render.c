...
void init() {
    ...
    init_potential_psi(&s_sim_frames, &s_sim_programs, &s_sim_params);
}

void render(RenderParams *render_params) {
    glViewport(0, 0, s_sim_params.texel_width, s_sim_params.texel_height);
    for (int i = 0; i < 6; i++) {
        timestep_schrod(&s_sim_frames, &s_sim_programs, &s_sim_params);
    }
    ...
    set_sampler2D_uniform("tex", s_sim_frames.potential_psi[0]);
    draw_unbind_quad();
}
