...
- **Edit Other Values** folder. This contains sliders to modify the mass **m** of the particle, and the time step **dt**. Note that modifying these values may incur instability in the simulation. The folder also contains the **Normalize** checkbox, that when checked the simulation is to normalize the wave function after each frame.
### Issues
- The simulation depends on the floating point numerical precision to be at least 32 bits; but to my knowledge, WebGL does not require for highp float to be at least 32 bit. If it is less than this on your machine, you may see the wave function quickly decay to zero.
